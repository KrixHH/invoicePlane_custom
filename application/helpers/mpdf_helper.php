...
function pdf_create(
    ...
    if ((!empty($CI->mdl_settings->settings['pdf_invoice_footer']) && $isInvoice) || $CI->load->view('/invoice_templates/pdf/footer_invoice', null, true) && $isInvoice ) {
        $mpdf->setAutoBottomMargin = 'stretch';
        $mpdf->SetHTMLFooter($CI->load->view('/invoice_templates/pdf/footer_invoice', null, true));
    }

    // Set the footer if voucher is quote and if set in settings
    if ((!empty($CI->mdl_settings->settings['pdf_quote_footer']) && strpos($filename, trans('quote')) !== false) || ($CI->load->view('/quote_templates/pdf/footer_quote', null, true) && strpos($filename, trans('quote')) !== false) ) {
       $mpdf->setAutoBottomMargin = 'stretch';
       $mpdf->SetHTMLFooter($CI->load->view('/quote_templates/pdf/footer_quote', null, true));
    }
